...
namespace {
    ...
    constexpr int kBaseW = 400, kBaseH = 296;
    ...
    // UI scale (1.0 = default). Set via ConfigUI_SetScale before the window is made.
    // S() scales a design-space pixel value to the actual on-screen value.
    float g_uiScale = 1.0f;
    static int S(int v) { return (int)(v * g_uiScale); }
#define kW S(kBaseW)
#define kH S(kBaseH)
    ...
    HWND Label(HWND p, const wchar_t* t, int y, COLORREF, int x = 18, int w = 80) {
        return CreateWindowExW(0, L"STATIC", t, WS_CHILD | WS_VISIBLE,
            S(x), S(y + 3), S(w), S(20), p, nullptr, nullptr, nullptr);
    }
    HWND Edit(HWND p, int id, int y, const std::wstring& val, bool password) {
        ...
        HWND e = CreateWindowExW(0, L"EDIT", val.c_str(), st,
            S(104), S(y), S(278), S(24), p, (HMENU)(INT_PTR)id, nullptr, nullptr);
        SetWindowTheme(e, L"", L"");   // classic rendering so WM_CTLCOLOREDIT applies
        return e;
    }

    void BuildControls(HWND h) {
        g_font = CreateFontW(-S(16), 0, 0, 0, FW_NORMAL, 0, 0, 0, DEFAULT_CHARSET,
            OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
            FF_DONTCARE, L"Segoe UI");
        g_fontHdr = CreateFontW(-S(20), 0, 0, 0, FW_SEMIBOLD, 0, 0, 0, DEFAULT_CHARSET,
            OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
            FF_DONTCARE, L"Segoe UI");

        HWND hdr = CreateWindowExW(0, L"STATIC", L"Archipelago", WS_CHILD | WS_VISIBLE,
            S(18), S(20), S(240), S(30), h, nullptr, nullptr, nullptr);
        CreateWindowExW(0, L"STATIC", L"F8 to close",
            WS_CHILD | WS_VISIBLE | SS_RIGHT, kW - S(150), S(26), S(132), S(18), h,
        ...
        g_btn = CreateWindowExW(0, L"BUTTON", L"Connect",
            ...
            S(115), S(210), S(170), S(32), h, (HMENU)IDC_TOGGLE, nullptr, nullptr);
    ...
    void ThreadMain() {
        ...
        if (g_wnd && IsWindow(g_wnd)) DestroyWindow(g_wnd);
        g_wnd = nullptr;
